...
// maxTokenBytes bounds a deposited token. A Dispact personal access token is
...
// connect deposits this member's token and records where their Dispact is.
//
...
func depositableToken(raw string) (string, error) {
	...
	switch {
	case token == "":
		return "", fmt.Errorf("%w: connecting needs the personal access token from your Dispact profile", extension.ErrInvalid)
