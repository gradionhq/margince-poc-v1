...
--
-- One row per connected member: where their Dispact is, how far the poll has
-- read, and whether the connection still works. It is applied by the pre-merge
-- gate as a restricted ext_dispact_connector role and at runtime by
...
CREATE TABLE ext.ext_dispact_connector_connection (
    ...
    -- WHICH Dispact. The bundle names one deployment, the product has others,
    ...
    -- other's gaps.
    CONSTRAINT ext_dispact_connector_connection_one_per_member
...
ALTER TABLE ext.ext_dispact_connector_connection ENABLE ROW LEVEL SECURITY;
ALTER TABLE ext.ext_dispact_connector_connection FORCE ROW LEVEL SECURITY;
...
CREATE POLICY ext_dispact_connector_connection_tenant_isolation
    ON ext.ext_dispact_connector_connection
...
GRANT SELECT, INSERT, UPDATE, DELETE ON ext.ext_dispact_connector_connection TO margince_app;
