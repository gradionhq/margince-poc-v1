...
// noteFailure), and that is deliberate: the Dispact screen and the Maintenance
...
// here. What it has instead, and zalo-oa does not, is a FLEET: many members
// polled in one tick, each failing on their own, which is where classEveryMemberFailed
// comes from.
...
var (
	...
	classTokenRejected = extension.FailureClass{
		...
		Sentence: "a member's stored Dispact token was refused, or no token is on deposit for them",
		Remedy:   "That member reconnects Dispact from their own Connections page and pastes a current token; nobody can deposit one on their behalf.",
	}
	...
	classProviderUnavailable = extension.FailureClass{
		...
		Sentence: "the Dispact server could not be reached, or answered that it was too busy",
	...
	classProviderAnswerUnusable = extension.FailureClass{
		...
		Sentence: "Dispact answered something this connector cannot read",
	...
	// stops their poll without anything about Dispact changing.
	classMemberNotPermitted = extension.FailureClass{
		...
		Remedy:   "Restore that member's role, or disconnect their Dispact connection if they are not meant to capture into the CRM any more.",
	}
	...
	classConnectionUnusable = extension.FailureClass{
		...
		Remedy:   "The member disconnects Dispact and connects it again: a poll cannot repair the connection record it was handed, and the refused value is in the process log.",
	}
	...
	classPollFailed = extension.FailureClass{
		Class:    "poll_failed",
		Sentence: "the Dispact poll failed for a cause this connector does not yet classify",
	...
	// unit polls many connections in one tick where zalo-oa polls exactly one.
	//
	...
	classEveryMemberFailed = extension.FailureClass{
		...
		Remedy:   "Read each connection's own class on the Dispact screen: the failures have nothing in common, so there is no single outage behind them.",
	}
)
