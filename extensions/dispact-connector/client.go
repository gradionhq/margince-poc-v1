...
// The provider half: three calls against one member's Dispact, and the bounds
...
const (
	...
	errUnauthorized dispactError = "dispact: the provider rejected this member's token"
	...
	errTransient dispactError = "dispact: the provider is unavailable"
...
type dispactError string

func (e dispactError) Error() string { return string(e) }
...
// client is one member's connection to one Dispact deployment.
type client struct {
	base  *url.URL
	token string
...
func refusePrivate(_, address string, _ syscall.RawConn) error {
	...
	if err != nil {
		return fmt.Errorf("dispact: the address %q is not host:port", address)
	}
	ip := net.ParseIP(host)
	if ip == nil {
		return fmt.Errorf("dispact: %q did not resolve to an address", host)
	}
	if !publicIP(ip) {
		return fmt.Errorf("dispact: refusing to dial %s — it is not a public address, and a member-supplied host must not become a probe of this deployment's own network", ip)
	}
	return nil
}
...
// providerUser is a Dispact account, as both /auth/me and the batch lookup
...
// me identifies the token's owner: which Dispact workspace this connection
