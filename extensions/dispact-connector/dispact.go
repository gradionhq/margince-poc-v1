...
// Package dispact is the ingress seam's consumer: a unit that pulls each
// connected member's Dispact inbox and lands what they were directed at on the
// CRM timeline, through the same capture pipeline a mailbox goes through.
//
// WHAT THIS UNIT IS NOT. The product's Dispact integration — cross-linking a
...
//
//   - migrations/ — ext_dispact_connector_connection, one row per connected
...
func New() extension.Extension {
	return extension.Extension{
		...
		Tools: []extension.Tool{
			...
			{Name: "dispact_status", Handle: status},
			{Name: "dispact_disconnect", Handle: disconnect},
		},
...
// core pairs with the unit name to derive `ext:dispact-connector:dispact`, the
