...
const connectionEntity = "ext_dispact_connector_connection"

// The verbs this unit publishes about its own rows. The type on the bus is
// `ext_dispact_connector.<verb>` — the core prefixes the namespace, so these
...
func recordConnection(ctx context.Context, tx extension.Tx, action extension.AuditAction, verb string,
	...
	if subject == nil {
		return fmt.Errorf("dispact: recording a %s needs one image — the row's id comes from whichever side of the write it has", verb)
	}
