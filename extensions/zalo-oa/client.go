...
func decodeEnvelope(body []byte, into any) error {
	...
	if err := json.Unmarshal(body, &envelope); err != nil {
		return fmt.Errorf("%w: the answer is not the envelope this unit reads", errProvider)
	}
	...
	if len(envelope.Data) == 0 {
		...
		// zero value as though the provider had said so.
		return fmt.Errorf("%w: the answer reported success and carried no data", errProvider)
	}
	if err := json.Unmarshal(envelope.Data, into); err != nil {
		return fmt.Errorf("%w: the answer's data is not the shape this unit reads", errProvider)
	}
	return nil
}
