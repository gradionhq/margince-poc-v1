...
## check-workflow-timeouts — every CI job carries a wall-clock ceiling. A job
## with no timeout-minutes inherits GitHub's SIX-HOUR default, so a hung one
## holds a required check for a working day while looking like a queue backlog.
## Derived from the workflow tree, so a job added later is covered the day it is
## committed.
