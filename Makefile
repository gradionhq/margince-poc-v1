...
## adr-index — regenerate docs/adr/README.md from the decision records
## themselves. The index is generated because a hand-maintained one drifts: the
## system this replaced kept an index and its records in two places and they
## disagreed about which decisions were live. backend/adrindex_test.go fails
## when the committed index is stale.
adr-index:
	@./scripts/gen-adr-index.sh
