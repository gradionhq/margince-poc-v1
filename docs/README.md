...
- [ingress-gate-and-auto-capture.md](explanation/ingress-gate-and-auto-capture.md) — the core-side half of ingress, connector-agnostic: the gate's refusal ladder and the two dispositions that both advance a cursor; auto-capture's one transaction (internal-only gate, append-once evidence, the activity write, the savepointed tier ladder T0–T4); and the verdict engine that answers what T4 deferred — sender kinds, the confidence floor, the approve-only review queue, hide-then-redact. Marks every stage ⚙️ deterministic or 🤖 AI with its task name, states what a dropped message does and does not store, and lists every threshold with where (or whether) it can be configured.
