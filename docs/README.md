...
- [build-the-desktop-app.md](how-to/build-the-desktop-app.md) — build the self-contained macOS or Windows folder that runs the whole stack with no Docker (`make desktop`), then run, configure and update an installation.
...
- [desktop-distribution.md](explanation/desktop-distribution.md) — the other shape: one folder a non-technical user runs on macOS with no Docker. Why it must carry its own Postgres (pgvector is not in `contrib`), how relocatability is enforced and verified, the update contract the folder layout encodes, single-file configuration, and the limits — collation, signing, and the socket-path ceiling.
