...
| `--mcp-apps-base-url` | `MARGINCE_MCP_APPS_BASE_URL` | api | the origin the api reads the MCP App view documents from (`GET <origin>/mcp-apps/<view>.html`), fetched once at startup and refreshed periodically. Defaults to `--public-base-url`, which the connector gate already makes a boot error to omit — so wherever `/mcp` is served the chain cannot be empty. The value must be **API-reachable**, which is not the same as publicly reachable: a container may lack ingress hairpin routing, external DNS or egress, and that is the case this setting exists for. A CDN origin is supported and recommended — it trades a dependency on the web tier for a better one rather than removing it. With the connector gate off, no fetch happens at all |
