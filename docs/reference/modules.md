...
| **capture** | The one `connector.Sink` — normalized inbound capture, one transaction per record, idempotent on `(source_system, source_id)` — plus the connector registry (grant-time scope intersection). | Sink + Registry (`NewSink`/`NewRegistry`; connector HTTP wired in compose) | `raw_capture, capture_connection, capture_exclusion_rule, capture_sync_state, capture_backfill, workspace_email_domain, capture_digest` | `/connectors` (+`/{provider}/connect`,`/callback`,`/disconnect`,`/backfill`), `/capture/exclusions`, `/digest` (via compose adapters) |
