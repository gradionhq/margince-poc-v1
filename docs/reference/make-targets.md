...
| `contract-frontend-drift` | The **third** regeneration a `backend/api/crm.yaml` change owes: `pnpm gen:api` writes `frontend/src/api/schema.d.ts` + `public-events.ts`, and until #1639 only the frontend lane enforced it — so a backend-only author could go green through the whole backend gate and strand the frontend types (#1573). Skips **loudly** when `pnpm` is absent, and **fails** rather than skipping when `CI`/`GITHUB_ACTIONS` is set. A `check-backend` prerequisite |
| `test-contract-frontend-drift` | `contract-frontend-drift`'s own test: the skip is loud, goes to stderr, cannot be taken in CI, and is the gate's only early exit. A gate that may skip is a gate that can skip silently, which is the defect it was written for. A `check-backend` prerequisite |
