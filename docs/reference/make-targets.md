...
| `license-module` (root) | Refresh the bundled license-validation wasm module (`backend/internal/platform/licensecheck/module/`) from the margince-constellation release it is pinned to: the tag is resolved against the commit it names, the download is verified against the digest GitHub reports for the stored asset, and the blob, `VERSION` and the `.sha256` are rewritten together. `make license-module TAG=sha-<commit>` moves the pin; a rolling tag is refused, since `latest` names a different build every day. Needs a `gh` login with access to that private repository — nothing else in the build does, because the module itself is committed. Compression is upstream's to change (it has moved from gzip to brotli once): the host reads the framing out of the bytes and the tree stores the artifact under one fixed name, so a refresh never touches Go code. A blob that stops matching its recorded digest fails `make check` |
| `tidy` | `go mod tidy` |
