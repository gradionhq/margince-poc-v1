...
| `pkg-freeze` | Published-surface freeze (ADR-0069 §3, EXT-P3): apidiff on every `backend/pkg` package vs the merge-base (the extensions integration branch while the arc holds there, else `origin/main`) — an incompatible change or removed published package fails, additive growth passes. A ratified change is its exact apidiff finding line in `scripts/pkg-freeze-allowlist.txt`, bound to the merge-base sha it was ratified against (a superseded entry can license nothing and warns until removed); ad-hoc baseline override: `PKG_FREEZE_BASE=<ref>` |
