...
| `check-workflow-timeouts` | Assert every job in `.github/workflows/` carries `timeout-minutes`. A job without one inherits GitHub's **six-hour** default, so a hang holds a required check for a working day and reads as a queue backlog rather than a failure while it does. Derived from the workflow tree, not a list of job names, so a job added later is covered the day it is committed. Pure Python + PyYAML |
