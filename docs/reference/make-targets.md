...
| `test-money-scale` | Points that gate at a throwaway tree and asserts thirteen cases: each language's divide, multiply and remainder fires (including an expression the formatter wrapped across lines), a percentage and a progress width do not, and the waiver silences its own line and only that line |
