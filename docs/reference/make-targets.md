...
| `e2e-ai-report` | Print the certification readiness report: one row per shipped invocation site (from the census, so a MISSING record is visible), its band, the accepted/wrong_answer/invalid/abstained counts behind it, the scope certified (`full_invocation` vs `single_turn`), and the (provider, model, env) it was measured on. A record whose stamp no longer matches the corpus renders `stale`, one that was never produced renders `absent` — never collapsed, and never as a row of zeroes. Reads `backend/internal/compose/aicert/{corpus,records}/`. `go run`-only dev tool (`internal/compose/aicert/reportcmd`), not a shipped binary and **not a merge gate**: it always exits 0, because the lane it reports on is paid and manual |
