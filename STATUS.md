...
- Open 2026-08-21 (draft): **the sign-in surface speaks the founder's copy, the settings sidebar drops the agent, settings verbs drop the ellipsis, and a tab click stops re-animating the page it is already on** ([#2194](https://github.com/gradionhq/margince-poc-v1/pull/2194), draft — CI skips a draft, `make check-fe` green locally: 3853 + 86 vitest, `tsc -b`, biome, the DS gates, `fe-build`). Four pieces of UI feedback, one sweep. **(1) The identity region on the sign-in surface** is five sentences in one voice — greeting, purpose, what the work is, the one promise, the handover to the form — replacing the four-limit list. Two bounds the region used to hold are lifted on purpose: it now opens with a GREETING (it admitted only limits and server-read facts) and it carries copy the TASK depends on (the handover exists to point at the form). The typed row is the greeting, the remaining rows fade up complete, and the reveal is retimed to 110ms per row for seven rows. `auth.coreBoundary`/`coreScope`/`corePermission`/`coreCites`/`coreWaits`/`coreMarks` are gone and `coreGreeting`/`corePurpose`/`coreWork`/`corePromise`/`coreHandover` replace them in all three catalogs. **The copy carries an em dash and a colon** — the house voice has been trimming dashes out of UI copy, and this text is quoted as written; if the voice rule wins, this is the line to change. **(2) `WorkspaceRail` renders no `.railfoot` at all on a drilled-in level**, so the agent is absent from the settings sidebar and unchanged on the top level: a section rail is navigation inside ONE destination and the agent belongs to the whole session. **(3) Ten settings row verbs lose the trailing ellipsis** (`password.open`, `settings.signatureEdit`, `agents.disconnectOpen`, `agents.revokeGrantOpen`, `settings.resetDataButton`, `stage.remove`, `overlay.userMap.map`/`change`, `settings.voice.addSourceOpen`/`addFirstOpen`), which **reverses the row-language rule** that a row's verb takes the ellipsis form. The ellipsis was carrying the disambiguation, so two dialog submits had to be renamed to keep it: `password.submit` → "Save new password" and `settings.resetDataConfirmButton` → "Reset everything". In-flight labels ("Saving…", "Minting…", "Building…"), input placeholders and the one progress HEADING keep theirs — an ellipsis that means *still going* is not the one that was complained about. Four in-source comments and five test comments that stated the old rule now state the current one: **the opener and the confirm are named for different acts, and a distinct name is what separates them.** **(4) The routed subtree is keyed on `routeIdentity()`** (`app/router.tsx`) — the address minus the trailing segments that only pick a VIEW of the same thing — instead of the whole address. A person-page tab click is now a re-render: the record header, the readings band, the rail and the tab strip the reader just clicked keep their DOM nodes, so `enter.css` plays the arrival for the panel that changed and nothing else, and the page's reads stop re-observing on every tab. Record → record still remounts, which is what the key is for. `IDENTITY_DEPTH` is total over `Screen`, so a screen that grows a URL tab has to declare itself; today `contacts` is the only one, and settings is explicitly NOT one (each entry is its own page). `App.tabkey.test.tsx` pins both halves at App level, where the key lives — `screens/personpage.test.tsx` mounts the page with a `tab` prop and can never see the router decide. **Two defects found on the way and fixed here:** `make seed-demo` never passed `MARGINCE_BLOBSTORE_REGION`, so the demo seeder aborted before writing a single company logo (`scripts/dev.sh` has always passed it — the Makefile leg had not); and `.auth-identity-col`'s `overflow: hidden` makes it a scroll container, whose automatic minimum size is ZERO, so the surface's auto grid row stopped asking for the height the identity copy needs and clipped it at stacked widths — `min-height: min-content` gives the row its content back and the surface scrolls, which is the rule that file opens with. **One follow-up left open:** on a company tab change both `section.panel` and its own `.panel-body` play the arrival, so that content fades through two multiplied fades — `enter.css` has a rule against exactly that for a nested `.arrive-stack`, and this pair slips past it. Not the reported bug, and not fixed here.
