...
- **`main` is red, and no branch can fix it from its own diff** ([#2255](https://github.com/margince/margince/issues/2255), `priority: critical`). Two failures from #2247: arch-lint refuses `dealrooms`' `yaml.v3` import (the fix is to move the contract-reading assertion to the root package, which the arch config's own comment argues for — not to widen `canUse`), and `migrations/testdata/head_catalog.txt` never learned the four Deal Room audit actions, so `TestMigrationsBuildTheCommittedSchema` fails. Every PR opened since inherits both.
