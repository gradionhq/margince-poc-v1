...
- Open, in review (2026-08-19): **the specification is retired, and the decisions that still bind moved here** ([#1774](https://github.com/gradionhq/margince-poc-v1/pull/1774), [#1783](https://github.com/gradionhq/margince-poc-v1/pull/1783), [#1790](https://github.com/gradionhq/margince-poc-v1/pull/1790), batch 4 to follow). The separate spec repository no longer decides anything: `CLAUDE.md` and `AGENTS.md` carry an ordered answer to what settles a question — the current request, then code and tests and `crm.yaml`, then the guardrails, then `docs/`, then the decision records — and say plainly that an older document describing a different choice is a conflict to name, not grounds to refuse work. This repository is public and now refers to **no private repository, document or link**; `TestPublicTreeCitesNothingPrivate` holds that with no exemption list, which is why `STATUS-ARCHIVE.md` left the tree rather than earning one. **56 decision records are being rewritten in plain language into `docs/adr/`**, numbers kept, each naming the files, tables and tests that make it checkable; `docs/adr/README.md` is generated and `backend/adrindex_test.go` fails a stale index, a missing status or a dangling supersede pointer. **The rule that shaped the migration: a decision the code has not caught up with stays in the record and names which half is missing, flagged in the status line.** Roughly a third carry such a flag, and reading them against the tree is what found the gaps below. Filed: [#1785](https://github.com/gradionhq/margince-poc-v1/issues/1785) (**the CRA 24-hour reporting duty went live 11 September 2026 and has no runbook**), [#1791](https://github.com/gradionhq/margince-poc-v1/issues/1791) (**the LICENSE says agents are not Seats and the seat count includes them** — one number, two texts, a decision for counsel), [#1784](https://github.com/gradionhq/margince-poc-v1/issues/1784) (notifications: claimed, never built, needs respecifying), [#1788](https://github.com/gradionhq/margince-poc-v1/issues/1788) (two modules can emit the same event type with no gate noticing), [#1769](https://github.com/gradionhq/margince-poc-v1/issues/1769) (`retention.applied.action` is an open string where the decision closed it). Also corrected in the rulebook itself: it claimed four `cmd/<role>` binaries where three exist, and a golangci `funlen` setting that is not configured.
