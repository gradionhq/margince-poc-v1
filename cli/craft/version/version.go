...
// (prompt, rubric, exemplar-set, model). The composed gate_version is stamped on
...
// Tuple is the four-part gate identity. Two of the parts are pinned in
// gate-version.json (prompt, model); the rubric and exemplar-set versions come
...
func (t Tuple) String() string {
	return fmt.Sprintf("p%s+r%s+e%s+%s", t.Prompt, t.Rubric, t.ExemplarSet, t.Model)
}
...
func Current() (Tuple, error) {
	...
	return Tuple{Prompt: p.Prompt, Rubric: r.Version, ExemplarSet: set.Version, Model: p.Model}, nil
}
