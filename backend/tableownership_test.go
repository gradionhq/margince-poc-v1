...
var crossStoreWrites = map[string]string{
	...
	"internal/compose/person360:user_record_view": "the person view's visit acknowledgement rides org360's table because since-last-visit is one fact per (user, record) — migration 0179 widened its entity_type CHECK to admit a person rather than adding a parallel table that would drift",
