...
var readAuthorityOnAWritePath = gatekit.Waive(map[string]string{
	...
	"internal/modules/commissions:VisibleClause":      "the READ spelling of a commission entry's deal-derived row scope, shared with the ledger page, the summary and the single read. Deciding an entry reads it through this before changing it, and a REVERSAL goes through WritableEntriesForDeal, which takes auth.EnsureWritable on the same deal",
