...
var readAuthorityOnAWritePath = gatekit.Waive(map[string]string{
	...
	"internal/modules/people:scopeOrAllRows":          "the shared spelling of a row-scope READ clause. Its one write-path reader is the lead-vocabulary writers' returned representation, whose lead_count is a count of the leads the CALLER MAY SEE — a read decision that only narrows. The vocabulary row being written is gated on its own object grant before the store runs, and no lead row is changed",
