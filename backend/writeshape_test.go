...
var auditOnlyWrites = map[string]string{
	...
	"internal/modules/activities:deleteAbsorbedEcho":     "the outbound-send reconcile folds the provider's own captured echo into the row the send wrote and hard-deletes it. activity.archived would be a lie — nothing was archived, the row is gone — and the closed catalog (events.md §5, shared/kernel/events/catalog.go) defines no hard-delete verb for an activity, which the closed-verb law forbids inventing build-side. The surviving row's own activity.updated, emitted in the same transaction, is what tells a read model to stop pointing at the identity that moved; the echo's already-relayed activity.captured cannot be un-delivered by any later event, so no event would repair what a subscriber holds",
