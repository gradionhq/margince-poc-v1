...
var auditOnlyWrites = map[string]string{
	...
	"internal/modules/activities:archiveAbsorbedEcho":    "the outbound-send reconcile folds the provider's own captured echo into the row the send wrote: the echo releases the natural key and goes off the timeline, deleting nothing. The closed catalog (events.md §5, shared/kernel/events/catalog.go) has no activity.merged, and the closed-verb law forbids inventing one build-side; activity.archived is the wrong fact rather than an available substitute — it announces a lifecycle disposition, and this is a de-duplication whose mechanism happens to be an archive. What a consumer actually needs is where the message lives now, which the surviving row's own activity.updated says in the same transaction. The echo's already-relayed activity.captured cannot be un-delivered by any later event, so no event would repair what a subscriber holds either",
