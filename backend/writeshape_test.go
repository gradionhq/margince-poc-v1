...
var auditOnlyWrites = map[string]string{
	...
	"internal/modules/ai:SetModelRate":                   "model prices are ratified audit-only (EVT-NOEVT-3) \u2014 the closed catalog (events.md \u00a75) defines no ai_model_rate.* type and the price sheet is workspace config recomputed price-on-read, the same ruling as the deals-owned product rate-card; inventing a build-side type or stream would violate the closed catalog (P3)",
	"internal/modules/deals:SetFxRate":                   "FX rates are ratified audit-only (EVT-NOEVT-3) \u2014 the closed catalog (events.md \u00a75) defines no fx_rate.* type and the rate sheet is workspace config recomputed price-on-read, the same ruling as the deals-owned product rate-card (CreateProduct is audit-only); an fx_rate.* verb on the deal stream would be a build-side invention the closed catalog forbids (P3)",
