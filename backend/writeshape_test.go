...
var auditOnlyWrites = gatekit.Waive(map[string]string{
	...
	"internal/modules/dealrooms:addDocumentTx":             "putting an attachment of the deal in front of the buyer. A room document is editorial exactly as a task definition is: buyer-visible only once a human publishes, and deal_room.published announces the manifest a buyer reads has changed. Emitting here would announce a document the buyer cannot yet see and, for a room never published, never will. The audit row answers \"who decided the buyer should see this file\"",
	"internal/modules/dealrooms:UpdateDocument":            "renaming, regrouping or reordering that same entry, audit-only on the identical ground: editorial, published by the next release. The audit image carries both sides of the change",
	"internal/modules/dealrooms:RemoveDocument":            "taking the entry out of the room, audit-only on the identical ground as ArchiveTask: the buyer stops seeing it at the next publish, the row survives so a release that named it still resolves, and the audit row names the attachment that was withdrawn",
	"internal/modules/dealrooms:createTaskTx":              "adding an item to a room's shared to-do list. A task DEFINITION is editorial: it is buyer-visible only once a human publishes, and deal_room.published is what announces that the list a buyer reads has changed. Emitting here would tell every subscriber the buyer was asked to do something at the moment a rep typed it into a draft, which is not true and, for a room that is never published, never becomes true. The one thing on this table that happens without a publish IS announced — deal_room.task_completion_changed fires from applyTaskPatch when either side ticks an item off — so the omission is the editorial/live split the table was built around rather than a missing verb. The audit row answers \"who put this on the buyer's list\", which is the question an item nobody agreed to raises",
	"internal/modules/dealrooms:ArchiveTask":               "taking an item off that same list, and audit-only on the identical ground as createTaskTx: removal is editorial and reaches a buyer through the next release, so announcing it would report a change nobody outside the seller's tab can yet see. The row survives rather than being deleted precisely so a completed item stays attributed, and the audit row carries the wording that was removed — which is what answers \"what was this item before somebody dropped it\"",
