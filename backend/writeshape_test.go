...
var auditOnlyWrites = gatekit.Waive(map[string]string{
	...
	"internal/modules/deals:SetBaseCurrency":             "the base currency is workspace config on the same surface as the rate sheet, which events.md ratifies audit-only (EVT-NOEVT-3) — the closed catalog defines no workspace-config verb and the closed-verb law forbids inventing one build-side. Nothing downstream can act on it either: the change is refused outright once any deal has frozen a rate against the old base, so no already-converted figure exists for a subscriber to revisit",
