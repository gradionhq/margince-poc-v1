...
var auditOnlyWrites = map[string]string{
	...
	"internal/modules/overlay:UpsertUserMap":                "the Margince-user <-> incumbent-user mapping is workspace config, not a record fact: mirror_user_map is not a public contract entity, and the closed catalog (events.md §5, shared/kernel/events/catalog.go) defines no mirror.user_* type. Emitting one would need a contract event with no consumer. Same audit-only posture as overlay's own x_sor_mode flip",
	"internal/modules/overlay:BlockAutoMap":                 "an admin's deliberate unmap is a workspace-config change on mirror_user_map, which is not a public contract entity — the closed catalog defines no mirror.user_* type. Same audit-only posture as UpsertUserMap",
	"internal/modules/overlay:revalidateEmailMapping":       "revoking a stale email-sourced mapping is a workspace-config change on mirror_user_map, which is not a public contract entity — the closed catalog defines no mirror.user_* type. Audited so a user's silently-removed access has a record; same posture as UpsertUserMap",
	"internal/modules/overlay:revokeEmailMappingsForOwners": "revoking a mapping whose incumbent owner email became ambiguous is a workspace-config change on mirror_user_map, which is not a public contract entity — the closed catalog defines no mirror.user_* type. Audited so a user's silently-removed access has a record; same posture as UpsertUserMap",
