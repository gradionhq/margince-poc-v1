...
var auditOnlyWrites = gatekit.Waive(map[string]string{
	"internal/modules/activities:StartExtractionReadQueued": "the run record for reading an attached document is operational reading STATUS, not a record fact — the closed catalog (events.md \u00a75) defines no attachment_extraction.* type and the closed-verb law forbids inventing one build-side. Waived on the same ground as StartTranscriptReadQueued directly below, with one difference that makes the case stronger rather than weaker: this reading stages no approval at all, so there is no downstream act to announce. What a human accepts out of it lands through acceptAttachmentExtraction, which writes the DEAL and emits the deal's own event. Nothing could act on an announcement sooner either — the only consumer is the job enqueued in this same transaction, which carries the row id. The audit row answers \"who asked for this document to be read\"",
	"internal/modules/activities:FinishExtractionRead":      "the same run record reaching a terminal state, written by the worker. Audit-only on the same ground: no attachment_extraction.* verb exists in the closed catalog, and the reading itself writes to no record — its fields sit on the row until a human accepts them, at which point the deal update emits deal.updated. Announcing the reading would report the arrival of an offer nobody has taken. The polling client reads the row itself, sooner than a subscriber could tell it. The audit row carries the outcome and how many fields were grounded, which is what a spend or quality question reads",
	"internal/modules/activities:StartTranscriptReadQueued": "the run record for reading a meeting transcript is operational reading STATUS, not a record fact — the closed catalog (events.md \u00a75) defines no transcript_read.* type and the closed-verb law forbids inventing one build-side. The reading is the same species as the deep-read dossier (createOrJoinSiteRead, below) and waived on identical ground: what the reading produces lands as staged approvals, each emitting its own event when it is accepted. Nothing downstream could act sooner either — the only consumer is the job enqueued in this same transaction, which carries the row id. The audit row answers \"who asked for this transcript to be read\"",
	...
	"internal/modules/signals:NarrowDerivedForActivity":     "the audience-change corrector narrowing a derived signal to its capture owner (or archiving an ownerless one). The closed catalog (events.md §5) carries signal.detected and signal.resolved, neither of which says \"this summary's audience narrowed\", and the closed-verb law forbids inventing one build-side — announcing the narrowing would also broadcast the existence of what a human just limited. Nothing downstream is left stale: the signal surfaces read the rows, and the same transaction drops the thread's scan watermark so the next extraction pass re-reads under the new audience. The audit row answers \"why did this summary vanish from my colleagues' view\"",
