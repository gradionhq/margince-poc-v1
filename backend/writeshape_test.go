...
var auditOnlyWrites = gatekit.Waive(map[string]string{
	...
	"internal/modules/capture:Add":                          "the workspace consumer-mail list is workspace config, ratified audit-only on the same ruling as the capture-settings write (EVT-NOEVT-3): the closed catalog (events.md \u00a75) defines no verb for a list entry, and the closed-verb law forbids inventing one build-side",
	"internal/modules/capture:Remove":                       "withdrawing a consumer-mail list entry, same ratified audit-only posture as Add",
