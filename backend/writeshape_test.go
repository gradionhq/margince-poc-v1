...
var auditOnlyWrites = map[string]string{
	...
	"internal/modules/ai:SetModelRate":                   "model prices are ratified audit-only \u2014 the closed catalog (events.md \u00a75) defines no ai/pricing stream to ride, so a rate write's only event home would be a build-side invented stream the closed-stream law forbids (the fx_rate sibling emits fx_rate.appended only because fx_rate is deals-owned and the deal stream exists; there is no ai equivalent)",
