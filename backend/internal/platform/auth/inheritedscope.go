...
func SignalScopeClause(ctx context.Context, alias string, arg func(any) int) (string, error) {
	...
	// signal for every bounded reader, not withhold it for a reason.
	...
	return fmt.Sprintf(`(%[6]s AND (%[1]s.entity_type IS NULL
	 ...
	 OR (%[1]s.entity_type = 'project'      AND EXISTS (SELECT 1 FROM project sj WHERE sj.id = %[1]s.entity_id AND %[5]s))))`,
		alias, person("sp"), organization("so"), deal("sd"), project("sj"), private), nil
}
