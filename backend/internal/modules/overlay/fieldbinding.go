...
const (
	// DispositionMapped: the mirror carries this field; Incumbent names the
	// source properties.
	DispositionMapped Disposition = "mapped"
	// DispositionDeferred: mappable from the incumbent, deliberately out of
	// scope for now. IssueURL is required — a deferral nobody tracks is a
	// gap that never closes.
	DispositionDeferred Disposition = "deferred"
	// DispositionUnmappable: the incumbent has no analogue at all.
	DispositionUnmappable Disposition = "unmappable"
	// DispositionNativeOnly: derived or server-stamped, so no mirror could
	// ever supply it (a version counter, a relationship strength computed
