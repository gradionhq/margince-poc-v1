...
// its recorded status.
func leadBehavioralSignals(ctx context.Context, tx pgx.Tx, leadID ids.UUID) ([]BehavioralSignal, error) {
...
func (w leadScoreRecompute) Apply(ctx context.Context, ev workflow.Event, eff workflow.Effect, _ *workflow.ApprovalToken) (workflow.RunResult, error) {
	leads, err := w.linkedLeads(ctx, ids.UUID(ev.Entity.ID))
	if err != nil {
...
func (w leadScoreRecompute) linkedLeads(ctx context.Context, activityID ids.UUID) ([]ids.UUID, error) {
	var leads []ids.UUID
