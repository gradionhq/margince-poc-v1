...
// live RBAC through the shared/ports/authz seam and refuses the firing if
// the owner can no longer perform the planned effect — never the author's
// stamped-at-authoring copy, and never a cached one.
//
...
func checkOwnerPermission(ctx context.Context, resolver authz.Resolver, ev workflow.Event, effect workflow.Effect) (gateDecision, error) {
	...
	if err != nil {
		if errors.Is(err, apperrors.ErrNotFound) {
			// The owner's live seat resolves to ErrNotFound for gone,
	...
	for _, action := range effect.Actions {
		if decision, blocked := checkActionPermission(rbac, ev, action); blocked {
			return decision, nil
		}
	}
	return gateDecision{}, nil
...
func checkActionPermission(rbac authz.RBAC, ev workflow.Event, action workflow.Action) (gateDecision, bool) {
	...
	if perm.Shape == PermissionTargetScoped {
		// The firing's real entity type is known now — this is exactly
		// why target-scoping defers its object resolution to fire time
		// (catalog_actions.go's PermissionTargetScoped doc). EntityType's
		// spellings already match the RBAC object vocabulary
		// (identity/internal/policy's coreObjects), so no translation
		// table is needed.
		if ev.Entity.Type == "" {
