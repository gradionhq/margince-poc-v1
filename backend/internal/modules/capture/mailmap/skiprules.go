...
func headerKeyword(value string) string {
	keyword, _, _ := strings.Cut(stripHeaderComments(value), ";")
...
func reachesGate(autoSubmitted string) bool {
	keyword := headerKeyword(autoSubmitted)
...
func isAutoReplyPrecedence(value string) bool {
	switch strings.ToLower(headerKeyword(value)) {
...
func isMachineTouched(autoSubmitted, precedence []string, autoresponderHeader bool) bool {
	...
	for _, v := range autoSubmitted {
		...
		if !strings.EqualFold(headerKeyword(v), notAutomatic) {
			return true
		}
	}
	for _, v := range precedence {
		...
		switch strings.ToLower(headerKeyword(v)) {
		case "bulk", "list", "junk":
			return true
		}
	}
