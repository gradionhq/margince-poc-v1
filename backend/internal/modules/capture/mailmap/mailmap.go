...
func isAutoReply(autoSubmitted, precedence string) bool {
	...
	// (`auto-replied; owner-email=…`), so match on the keyword alone.
	keyword, _, _ := strings.Cut(autoSubmitted, ";")
	switch strings.ToLower(strings.TrimSpace(keyword)) {
	case "", "no":
		// Not automatic at all: an ordinary message a person wrote.
	case autoGenerated:
		// The signed envelopes and notices ADR-0072 §1 keeps on the timeline.
	default:
		// Every other value — `auto-replied`, `auto-notified`, and any
		// extension token a future RFC or a provider invents — is treated as a
		// reply. Unknown means unknown, and the safe reading of an unknown
		// automatic message is the one that cannot buy a T1 spare.
		return true
	}
