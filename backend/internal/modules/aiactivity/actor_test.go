...
func TestAHumanActorIsResolvedFromTheActorID(t *testing.T) {
	user := ids.NewV7()
	scope, got, err := aiactivity.ResolveActor(events.Actor{Type: "human", ID: "human:" + user.String()})
	if err != nil {
		t.Fatalf("ResolveActor: %v", err)
	}
	if scope != aiactivity.ScopePersonal || got != user {
...
func TestAnAgentActorIsResolvedFromOnBehalfOf(t *testing.T) {
	user := ids.NewV7()
	scope, got, err := aiactivity.ResolveActor(events.Actor{
		Type: "agent", ID: "agent:" + ids.NewV7().String(), OnBehalfOf: &user,
	})
	if err != nil || scope != aiactivity.ScopePersonal || got != user {
...
func TestASystemActorWithNoHumanIsWorkspaceScoped(t *testing.T) {
	scope, got, err := aiactivity.ResolveActor(events.Actor{Type: "system", ID: "system:relay"})
	if err != nil || scope != aiactivity.ScopeWorkspace || !got.IsZero() {
...
func TestAnUnparseableHumanActorIsRefused(t *testing.T) {
	for _, id := range []string{"human:not-a-uuid", "human:", "", ids.NewV7().String()} {
		if _, _, err := aiactivity.ResolveActor(events.Actor{Type: "human", ID: id}); err == nil {
...
func TestAHumanActorActingOnBehalfOfSomebodyElseIsRefused(t *testing.T) {
	...
	if _, _, err := aiactivity.ResolveActor(events.Actor{
