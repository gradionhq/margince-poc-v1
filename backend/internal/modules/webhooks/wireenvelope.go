...
// webhook subscriber receives (public-events.yaml's WebhookDeliveryEnvelope,
...
func toWireEnvelope(env kevents.Envelope) (crmcontracts.WebhookDeliveryEnvelope, error) {
	var data map[string]interface{}
	if len(env.Payload) > 0 {
		if err := json.Unmarshal(env.Payload, &data); err != nil {
			// env.Payload is JSON this same process marshaled moments earlier
			// from a typed WebhookPayload* struct (the module stores' emit
			...
			return crmcontracts.WebhookDeliveryEnvelope{}, fmt.Errorf("webhooks: decoding payload for event %s as the public data field: %w", env.EventID, err)
		}
	}
	return crmcontracts.WebhookDeliveryEnvelope{
		...
		Actor: crmcontracts.WebhookActor{Type: env.Actor.Type},
		Entity: crmcontracts.WebhookEntityRef{
