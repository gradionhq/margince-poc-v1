...
func TestActivityUpdateArchiveRelink(t *testing.T) {
	...
	if status := e.call(t, "POST", "/v1/workspaces", anyMap{
		"workspace_name": "Act E2E", "admin_email": "act@fable.test",
		"admin_display_name": "Act Admin", "admin_password": "correct-horse-battery",
	}, nil, nil); status != http.StatusCreated {
		t.Fatalf("bootstrap → %d", status)
	}
	if status := e.call(t, "POST", "/v1/auth/login", anyMap{
		"email": "act@fable.test", "password": "correct-horse-battery",
	...
	if status := e.call(t, "PATCH", "/v1/activities/"+task.ID, anyMap{"is_done": true}, nil, &updated); status != http.StatusOK {
	...
	if status := e.call(t, "PATCH", "/v1/activities/"+task.ID, anyMap{"subject": "x"},
	...
	for i := 0; i < 2; i++ {
		if status := e.call(t, "POST", "/v1/activities/"+task.ID+"/relink", anyMap{
	...
	if err := e.owner.QueryRow(t.Context(),
		`SELECT count(*) FROM activity_link WHERE person_id = $1`, person.ID).Scan(&links); err != nil {
		t.Fatal(err)
	}
	if links != 1 {
	...
	if status := e.call(t, "POST", "/v1/activities/"+task.ID+"/relink", anyMap{
		"entity_type": "lead", "entity_id": person.ID,
	}, nil, nil); status != 422 {
	...
	// mutations refuse.
	if status := e.call(t, "DELETE", "/v1/activities/"+task.ID, nil, nil, nil); status != http.StatusOK {
		t.Fatalf("archive → %d", status)
	}
	var archived struct {
		ArchivedAt *string `json:"archived_at"`
	}
	if status := e.call(t, "GET", "/v1/activities/"+task.ID, nil, nil, &archived); status != http.StatusOK || archived.ArchivedAt == nil {
		t.Fatalf("archive did not stamp: %d %+v", status, archived)
	}
	if status := e.call(t, "PATCH", "/v1/activities/"+task.ID, anyMap{"subject": "zombie"}, nil, nil); status != http.StatusNotFound {
