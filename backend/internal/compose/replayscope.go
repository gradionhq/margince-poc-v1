...
var replayableOperations = map[string]replayTarget{
	...
	// The replayed body is DealRoomInvitationIssued, whose record is nested
	// under "participant" — and replaying it returns the ORIGINAL credential,
	// which is exactly right: a retry that minted a second one would silently
	// retire the link already in the recipient's mailbox.
	"POST /v1/deal-rooms/{id}/participants": {object: probeDealRoom, moduleProbe: probeDealRoom, pathParam: "id", rowNote: "a participant's visibility is its room's, which is its deal's; the dealrooms store owns the probe"},
