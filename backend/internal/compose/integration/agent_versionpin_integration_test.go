...
//
// The version pin used to be taken server-side for exactly the five
// datasource-readable record types, and to fall back to the caller's own
// If-Match header for everything else — a header the contract declares
// optional. An agent that simply left it off staged target_version NULL, and
// the redemption skew check short-circuits on NULL, so the approval
// authorized the operation against whatever the row had drifted to inside
// the TTL. `offer` is the sharp case: POST /v1/offers/{id}/send carries no
// body, so its diff_hash is a CONSTANT for that offer id, while the
// line-item routes underneath it are auto_execute — the agent could rewrite
// the priced terms between the human's approval and the redemption and ship
// the offer at its own number.
//
// This drives that exact sequence over real HTTP.
...
func TestApprovedOfferSendRefusesAfterTheAgentRewritesTheTerms(t *testing.T) {
	e := setup(t)
	e.slug = "offers-pin"
	...
	// Stage the send WITHOUT If-Match — the omission that used to defeat the
	// pin entirely.
	var problem struct {
		Code   string `json:"code"`
		Detail string `json:"detail"`
	}
	if status := e.call(t, "POST", "/v1/offers/"+offer.ID+"/send", nil, bearer, &problem); status != http.StatusForbidden ||
	...
	if status := e.call(t, "POST", "/v1/offers/"+offer.ID+"/send", nil, withToken, &refusal); status != http.StatusConflict ||
