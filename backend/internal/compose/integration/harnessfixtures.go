...
// The STORE-MEDIATED record fixtures, and the id wideners the suites reach them
// through. seed.go holds the other half — everything that writes its rows with
// its own INSERT — and the line between the two files is that distinction, not
// the size of either.
//
// Split out of harness.go when that file crossed the 500-line cap. harness.go
// keeps what every suite needs before it can act at all: the environment, the
// pool, and the principals. What is here is what a suite reaches for once it
// has those, which is a different question and a different reason to open a
// file.
