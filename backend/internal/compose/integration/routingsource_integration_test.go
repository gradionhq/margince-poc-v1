...
// A deployment declares its binding in margince.yaml and the installation
// serves it from first boot — no API call, no routing file.
//
// This is what lets the routing file retire without costing an
// infrastructure-as-code deployment its declarative bootstrap: the binding is
// in the file the deployment already reviews, consumed exactly once, and the
// database is authoritative afterwards.
func TestADeclaredBindingIsSeededAtBootstrapAndOutranksTheFile(t *testing.T) {
	e := SetupSearch(t)
	ctx := e.adminRoutingCtx()

	// What bootstrap does with a declared seed, run against the same
	// transaction shape the installation path uses.
	declared := parsedRouting(t, "declared")
	if err := database.WithWorkspaceTx(ctx, e.Pool, func(tx pgx.Tx) error {
		_, err := settings.SeedValue(ctx, tx, ai.Routing, declared)
		return err
	}); err != nil {
		t.Fatalf("seeding the declared binding: %v", err)
	}

	// A routing FILE is present and says something different. The stored
	// binding must win: the file's only remaining job is seeding an
	// installation that has none, and this one has one.
	path := writeRouting(t, offlineRouting)
	got, err := compose.ResolveRouting(ctx, e.Pool, path, config.Static(nil), discard())
	if err != nil {
		t.Fatalf("ResolveRouting: %v", err)
	}
	if got.RoutingVersion() != declared.RoutingVersion() {
		t.Errorf("resolved %q, want the declared %q — the file outranked a binding the installation already holds",
			got.RoutingVersion(), declared.RoutingVersion())
	}
	if m, ok := got.Tiers[ai.TierPremium]; !ok || m.Model != "declared" {
