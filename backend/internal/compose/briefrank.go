...
// The deterministic Morning-Brief ranker (B-E05.1, formulas-and-rules
// §10/§10.1) over real rows: the candidate query, the per-deal evidence
// gathering, and the warmth seam. It lives in compose because it is a
// cross-module composition — deal facts (deals), relationship warmth
// (people §4), and the overnight activity signal (activities) — exactly
// the report engine's charter. The pure fold it feeds is briefscore.go;
// the persisted read model is briefstore.go. The composite is the
