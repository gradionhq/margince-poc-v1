...
// (cold-start read-back, brief re-rank, offer drafting) consume: they
// call the model once and use only its Response, never the runner's
// per-step Meta. Both routerBrain and the offline *ai.FakeClient satisfy
// it, so the fake path needs no adapter on these lanes. Only the Surface-B
// runner lane (Agent) needs runner.Brain's Meta return.
...
func FakeModelPath(client *ai.FakeClient) ModelPath {
	return ModelPath{Agent: fakeBrain{client: client}, ColdStart: client, BriefRank: client, OfferDraft: client, Embedder: client}
}
