...
-- The redaction sweep joins captured mail to its sender's disposition, so it
-- looks the noise rows up by address. There is deliberately no "already
-- redacted" flag to index: the absence of unredacted mail IS the completed
-- state, which is what lets mail arriving after a verdict still be covered.
CREATE INDEX idx_capture_pending_counterparty_noise
  ON capture_pending_counterparty (email)
  WHERE status = 'noise';
