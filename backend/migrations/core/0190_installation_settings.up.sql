...
--
-- Scoped to the live workspace (archived_at IS NULL), the same row the runtime
-- resolves as the installation (ADR-0061 §3).
INSERT INTO setting (key, value)
SELECT 'installation.name', to_jsonb(w.name)
  FROM workspace w WHERE w.archived_at IS NULL
