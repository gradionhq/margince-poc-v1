...
-- one every restored row belonged to. The shape is restored, not a per-row
-- reconstruction: nothing else records which workspace a user was in.
SET LOCAL lock_timeout = '3s';

ALTER TABLE app_user ADD COLUMN workspace_id uuid;
UPDATE app_user SET workspace_id = (SELECT id FROM workspace ORDER BY created_at LIMIT 1);
...
UPDATE session SET workspace_id = (SELECT id FROM workspace ORDER BY created_at LIMIT 1);
ALTER TABLE session
  ALTER COLUMN workspace_id SET NOT NULL,
