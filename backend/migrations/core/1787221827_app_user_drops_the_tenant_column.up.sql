...
-- these two serve the roster's live-user scan and the session lookup every
-- authenticated request makes. Narrow built before wide is dropped, so no read
...
CREATE INDEX idx_app_user_live ON app_user (id) WHERE archived_at IS NULL;
