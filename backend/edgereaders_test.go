...
var predicateEdgeReads = gatekit.Waive(map[string]string{
	...
	"internal/modules/activities/projectcoverage.go": "the stakeholder arm of the project filing-coverage count: the edge decides which activities COUNT as circling the project, and no edge column or endpoint pair reaches the caller — only a number does, over the caller's own activity row scope. Removing the arm would under-count the filing debt, never widen what is disclosed. The cost is that the count weakly reflects that some seat exists on the project",
