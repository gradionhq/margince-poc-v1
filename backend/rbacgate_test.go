...
var ungatedEntryPoints = gatekit.Waive(map[string]string{ // #nosec G101 -- waiver rationales for the fitness gate, not credentials
	...
	"internal/modules/dealrooms:DecideAsBuyer":                "records a decision on the version the latest release names for a document of the session's room, attributed to the session's participant; needs the reviewer capability and, for a confirmation, no open required-change thread",
	...
	"internal/modules/identity:ListRoles":             "gated by the explicit Identity parameter: hasRole(admin) refuses before any read. auth.Require is the wrong gate here in a way worth naming — \"role\" is deliberately absent from policy.coreObjects, and it could not be added without deciding what a grant on the RBAC vocabulary itself would mean; administering the permission model is installation-wide administration, which is exactly what RequireAdmin/hasRole(admin) exists for (see auth.RequireAdmin's own comment)",
	"internal/modules/identity:SetRoleObjectGrant":    "gated by the explicit Identity parameter: hasRole(admin) refuses before the vocabulary check and before the transaction opens, same rationale as ListRoles. Gating the editor of the grant map on a grant in that same map would also be circular: an admin who revoked their own grant could never restore it",
	...
	"internal/modules/identity:CreateTeam":            "gated by the explicit Identity parameter: hasRole(admin) refuses before any read or write",
	"internal/modules/identity:UpdateTeam":            "gated by the explicit Identity parameter: hasRole(admin) refuses before any read or write",
	"internal/modules/identity:SetTeamMember":         "gated by the explicit Identity parameter: hasRole(admin) refuses before any read or write",
	"internal/modules/identity:PreviewAccess":         "gated by the explicit Identity parameter: hasRole(admin) refuses before any read; the evaluation reads role documents and team names, nothing a member owns",
	"internal/modules/identity:UserAccess":            "gated by the explicit Identity parameter: hasRole(admin) refuses before any read; the evaluation reads role documents and team names, nothing a member owns",
...
var entryPointsOutsideModules = gatekit.Waive(map[string]string{
	...
	"internal/compose/dealbrief/service.go":           "dealbrief.Service.Get — the deal's standing brief. It gathers its facts through gated reads and nothing else: deals.Store.GetDeal and DealHealth (deal.read plus the deal row scope), activities.Store.ListActivities and ListOpenTasks (activity.read plus the link-walk scope, narrowed to the deal), dealrooms.Store.ListRooms, ListThreads and ListDecisions (deal_room.read plus the deal scope; a caller without the grant reads no room and gets no room section). The fold over them (write) is a pure function of what those reads returned, and it writes nothing",
	"internal/compose/nextaction/service.go":          "nextaction.Service.Get — the deal's next-best-action read. It gathers its facts through three gated reads and nothing else: deals.Store.GetDeal (deal.read plus the deal row scope), activities.Store.ListActivities and ListOpenTasks (activity.read plus the link-walk scope, narrowed to the deal). The fold over them (decide) is a pure function of what those reads returned, so a record the caller cannot see never reaches it, and it writes nothing — the verb it names is performed by the client through that verb's own door, which takes its own gates",
	"internal/compose/meetingbrief/service.go":        "meetingbrief.Service.Get — the pre-meeting brief read. UNLIKE its personbrief sibling it carries no auth.RequireHuman: agents read it through prep_for_meeting, so that one tool and the person page cannot answer the same question differently. What admits the read instead is auth.Require on activity and person, then auth.EnsureActivityContentVisibleLive on the meeting itself, and every record it is written from arrives through the caller's own gated 360 and the people store's own claim read, so the row-scope gates run in those reads rather than here. An agent is capped the same way a person is: a passport's authority is re-derived from the granting human's live seat on every call, never read off the principal. It also writes nothing — there is no cache — so there is no mutation for this gate to be the last line in front of",
