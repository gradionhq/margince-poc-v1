...
var ungatedEntryPoints = map[string]string{ // #nosec G101 -- waiver rationales for the fitness gate, not credentials
	...
	"internal/modules/identity:StartOIDCLogin":        "pre-principal by design (A107/ADR-0061 §6): stores one outbound authorization attempt for a human who is not signed in yet; the row names no user and reads nothing",
	"internal/modules/identity:ClaimOIDCLoginState":   "pre-principal: possession of the state (row + browser cookie, single-use) IS the authority being verified, exactly as the reset token is for A74",
	"internal/modules/identity:CompleteOIDCLogin":     "pre-principal: the provider's validated ID token is what admits the actor — the federated twin of Login, and the only thing it may resolve is a human the (issuer, subject) binding or a verified-email match already names",
	...
	"internal/modules/search:SweepWorkspaceEmbeddingDrift":    "periodic worker sweep (ADR-0069 §3a): heals identity-matched embedding gaps under the system principal, same posture as ReembedCorpus — no request, no human actor to gate",
