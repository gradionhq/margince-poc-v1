...
var ungatedEntryPoints = gatekit.Waive(map[string]string{ // #nosec G101 -- waiver rationales for the fitness gate, not credentials
	...
	"internal/modules/identity:MintSetupToken":               "pre-authentication by construction (ADR-0105): it issues the credential a claim is made WITH, on an installation that holds no organization, therefore no roles, no users and nobody who could hold a grant. What bounds it is the caller: the composition root at boot, and the operator CLI. What bounds ABUSE is the single-outstanding index — a second call while one is live is refused rather than issuing a rival credential",
	...
	"internal/modules/activities:LabeledCaptureCountSince":    "aggregate count read (ADR-0068 cost pre-flight) consumed only by the compose estimator; it returns a single labeled-message COUNT, never row data, so there is no row whose visibility a gate could decide (same shape as the approvals existence probes)",
