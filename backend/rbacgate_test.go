...
var ungatedEntryPoints = gatekit.Waive(map[string]string{ // #nosec G101 -- waiver rationales for the fitness gate, not credentials
	...
	"internal/platform/settings:RawTx":                       "reads one installation-wide setting inside a transaction the caller already holds, for a caller that needs the VALUE to finish work its own entry point already admitted — the base currency an amount converts to, the zone a period is computed in. It replaces a column read on the installation row that took no gate either (ADR-0091 phase 4), so gating it now would newly refuse the worker sweeps that have no human principal to grant against. Raw, the surface that serves the setting AS the answer, stays gated per entry",
