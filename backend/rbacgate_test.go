...
var ungatedEntryPoints = map[string]string{ // #nosec G101 -- waiver rationales for the fitness gate, not credentials
	...
	"internal/modules/approvals:WasDeclined":                  "the already-refused half of the same existence probe, consumed by the same sweeps before they stage: it answers one boolean about an offer the caller itself derived (kind + target + its own diff hash) and returns no record data — a sweep asking whether the thing it is about to propose was already declined carries no human principal to admit",
