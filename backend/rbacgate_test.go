...
var ungatedEntryPoints = gatekit.Waive(map[string]string{ // #nosec G101 -- waiver rationales for the fitness gate, not credentials
	...
	"internal/modules/identity:ActorIdentity":                "answers who the CALLER is, and nothing else: it reads display_name and email off the one app_user row the principal already authenticated as, resolving UserID then OnBehalfOf so an agent writes as the human whose authority it holds. There is no object to grant on for the same reason SeatNames has none — a seat is not a record — and this is one step narrower than SeatNames, which names colleagues where this names only the asker. Nothing here is disclosure: the caller presented these credentials to make the call. app_user carries FORCE RLS (core 0014), so the transaction's workspace binding is what stops a seat in another tenant being named however the principal was built. A principal with no human behind it, and a seat this workspace does not hold, both answer empty rather than erroring, because an unsigned draft is the specified outcome there (DRAFT-AC-E-6)",
