...
var ungatedEntryPoints = map[string]string{ // #nosec G101 -- waiver rationales for the fitness gate, not credentials
	...
	"internal/modules/people:SetChannelIdentityBlocked":       "reachability bookkeeping driven by the telegram ingest worker under the workspace-channel connector principal, never a human caller: the trigger is Telegram's own my_chat_member delivery, and admission happened at the webhook that verified the per-connection secret and resolved a status='connected' row. It writes no record data — only blocked_at on an identity the same delivery names — and RLS scopes the write to the resolved workspace",
	"internal/modules/people:EnqueueIdentityConflict":         "system observation raised by the ingress ensure path under the same connector principal (design §7.3): an inbound message whose exact lanes disagree records the (A, B) pair for human review. There is no human to gate — the customer's message is the trigger — and it writes no key onto either person, so it cannot merge them; the human authority sits on the queue's own disposition, not on recording that a disagreement exists",
	...
	// RecordFailure/RecordDeferral/MarkInFlight/ClearInFlight are the
	// dispatcher's own state-machine steps, driven by the outbox/River worker
