...
var ungatedEntryPoints = map[string]string{ // #nosec G101 -- waiver rationales for the fitness gate, not credentials
	...
	"internal/modules/approvals:HasDeclinedFor":               "existence probe consumed by gated sibling flows (the sweep asking whether a human already refused this proposal before applying it); returns no record data — the same shape as HasPendingFor/HasPendingKind",
	"internal/modules/approvals:HasDeclinedForTx":             "transactional form of HasDeclinedFor so the refusal check and the caller's write commit as one unit; same probe, same absence of record data, and the caller supplies only the commit boundary",
