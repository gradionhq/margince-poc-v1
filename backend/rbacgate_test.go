...
var ungatedEntryPoints = gatekit.Waive(map[string]string{ // #nosec G101 -- waiver rationales for the fitness gate, not credentials
	...
	"internal/modules/activities:ReconcileExtractionActivity": "worker path (compose/jobs_aiactivity.go): the pass re-announces the CURRENT state of readings the AI-activity projection may be stale about, under the system principal. It returns nothing and changes no reading — it re-publishes what the rows already say — so there is no decision here for a grant to govern, and a row-scope gate would narrow the repair to one rep's readings and leave every other person's display permanently wrong. Attribution is not lost by being ungated: each event it stages carries the reading's own actor, resolved by the projection from the envelope, and the projection is what decides who ever sees the row",
...
var entryPointsOutsideModules = gatekit.Waive(map[string]string{
	...
	"internal/compose/meetingbrief/service.go":        "meetingbrief.Service.Get — the pre-meeting brief read, ratified on the same terms as its personbrief sibling: it opens with auth.RequireHuman, gates the meeting itself with auth.EnsureActivityContentVisibleLive, and every record it is written from arrives through the caller's own gated 360 and the people store's own claim read, so the row-scope gates run in those reads rather than here. It also writes nothing — there is no cache — so there is no mutation for this gate to be the last line in front of",
	...
	"internal/compose/agentactivity/store.go":         "agentactivity.Store.Mine — the personal agent-activity read. It writes nothing, so there is no mutation for this gate to be the last line in front of, and its row scope is not an RBAC object but the attribution predicate itself: all three statements join passport and filter on passport.on_behalf_of = the authenticated caller, so a run this person did not authorize is not reachable through it, and a run whose passport was deleted (ON DELETE SET NULL) belongs to nobody rather than to whoever asks. The transport refuses a principal with no user identity before the store is called, and the operation is x-agent-access: human-only, which the generated agent-policy table enforces. Ratified here only as a subject the roots do not cover",
