...
var ungatedEntryPoints = map[string]string{ // #nosec G101 -- waiver rationales for the fitness gate, not credentials
	...
	"internal/modules/people:SetChannelIdentityBlocked":       "reachability bookkeeping driven by the telegram ingest worker under the workspace-channel connector principal (compose/telegramingest.go builds that principal onto the context before it classifies the update, so this write has a named actor), never a human caller: the trigger is Telegram's own my_chat_member delivery, and admission happened at the webhook that verified the per-connection secret and resolved a status='connected' row. It writes no record data — only blocked_at on an identity the same delivery names — RLS scopes the write to the resolved workspace, and the flip is not silent: the audit row and person.updated event it commits alongside are stamped from that same principal, so a reachability change is traceable to the delivery that caused it",
