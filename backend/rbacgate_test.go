...
var ungatedEntryPoints = gatekit.Waive(map[string]string{ // #nosec G101 -- waiver rationales for the fitness gate, not credentials
	...
	"internal/modules/overlay:RecordReprojectionFailure":     "the re-projection sweep's own bookkeeping about the pass it just failed, on the same worker system-principal path as Ingest and StaleProjections. It writes one column of the mirror row it was handed — the declaration fingerprint the re-fetch could not reach — and no incumbent payload, so it discloses nothing and returns no record; the row it names came from StaleProjections inside this workspace's own bound transaction, so there is no row whose visibility a gate could decide",
