...
func TestMergePerson_relinkSurvivorshipAndReferentialIntegrity(t *testing.T) {
	...
	firstAda := "Ada"
	source, err := e.store.CreatePerson(admin, CreatePersonInput{
		FullName: "Ada Source", FirstName: &firstAda, Source: "manual",
		Emails: []PersonEmailInput{{Email: "ada.work@x.test", EmailType: "work", IsPrimary: true}},
	})
	if err != nil {
		t.Fatalf("create source: %v", err)
	}
	target, err := e.store.CreatePerson(admin, CreatePersonInput{
		FullName: "Ada Target", Source: "manual",
		Emails: []PersonEmailInput{{Email: "ada.other@x.test", EmailType: "work", IsPrimary: true}},
	})
	if err != nil {
	...
	survivor, err := e.store.MergePerson(admin, src, tgt)
	if err != nil {
	...
	after, err := e.store.GetPerson(admin, tgt, false)
	if err != nil {
	...
	if _, err := e.store.GetPerson(admin, src, false); err == nil {
		t.Error("merged-away source still reads as live")
	}
	archived, err := e.store.GetPerson(admin, src, true)
	if err != nil {
...
func TestMergePerson_consentMergesRestrictively(t *testing.T) {
	...
	if _, err := e.store.MergePerson(admin, source, target); err != nil {
		t.Fatalf("merge: %v", err)
	}
...
func TestMergeOrganization_hierarchyReparenting(t *testing.T) {
	...
	source, err := e.store.CreateOrganization(admin, CreateOrganizationInput{DisplayName: "Acme Source", Source: "manual"})
	if err != nil {
		t.Fatalf("create source: %v", err)
	}
	target, err := e.store.CreateOrganization(admin, CreateOrganizationInput{DisplayName: "Acme Target", Source: "manual"})
	if err != nil {
		t.Fatalf("create target: %v", err)
	}
	srcID, tgtID := ids.UUID(source.Id), ids.UUID(target.Id)
	// A child sits under the source.
	child, err := e.store.CreateOrganization(admin, CreateOrganizationInput{
	...
	if _, err := e.store.MergeOrganization(admin, srcID, tgtID); err != nil {
		t.Fatalf("merge: %v", err)
	}

	// The child is re-homed under the survivor.
	got, err := e.store.GetOrganization(admin, ids.UUID(child.Id), false)
	if err != nil {
...
func TestMergeOrganization_partnerExtensionMovesIntoVacancy(t *testing.T) {
	...
	source, err := e.store.CreateOrganization(admin, CreateOrganizationInput{DisplayName: "Partner Source", Source: "manual"})
	if err != nil {
		t.Fatalf("create source: %v", err)
	}
	target, err := e.store.CreateOrganization(admin, CreateOrganizationInput{DisplayName: "Plain Target", Source: "manual"})
	if err != nil {
		t.Fatalf("create target: %v", err)
	}
	srcID, tgtID := ids.UUID(source.Id), ids.UUID(target.Id)
	...
	got, err := e.store.GetOrganization(admin, tgtID, false)
	if err != nil {
		t.Fatalf("read survivor: %v", err)
	}
...
func TestMerge_errorPaths(t *testing.T) {
	...
	// Self-merge.
	var selfErr *MergeSelfError
	if _, err := e.store.MergePerson(admin, a, a); !errors.As(err, &selfErr) {
		t.Fatalf("self-merge → %v, want MergeSelfError", err)
	}

	// First merge succeeds; a second merge OF the same source answers
	// AlreadyMerged with the redirect pointer.
	if _, err := e.store.MergePerson(admin, a, b); err != nil {
		t.Fatalf("first merge: %v", err)
	}
	var already *AlreadyMergedError
	if _, err := e.store.MergePerson(admin, a, c); !errors.As(err, &already) {
		t.Fatalf("re-merge of a merged-away source → %v, want AlreadyMergedError", err)
	...
	var deadTarget *MergedTargetError
	if _, err := e.store.MergePerson(admin, c, a); !errors.As(err, &deadTarget) {
		t.Fatalf("merge into a dead target → %v, want MergedTargetError", err)
	}
}
