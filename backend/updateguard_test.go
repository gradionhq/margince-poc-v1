...
var unguardedByIDUpdates = gatekit.Waive(map[string]string{
	...
	"internal/modules/privacy:expireRestriction":    "the `restricted_at IS NOT NULL AND restricted_until <= now()` predicate with RETURNING is the CAS: a rival sweep that already completed the row matches nothing, pgx.ErrNoRows ends the transaction, and nothing is audited twice for one erasure",
	"internal/modules/privacy:ReleaseRestriction":   "the `restricted_at IS NOT NULL` predicate with RETURNING is the CAS: a release racing the expiry sweep or a second administrator matches nothing and answers 409 rather than erasing and auditing the same record twice. A version guard would be wrong here — a held record has no optimistic-concurrency editor, and the deciding administrator is acting on a list, not on a version they read",
