...
// requests inside a bounded window and stops the relay lane after the server,
// so late-committing requests usually ship before exit.
func serveUntilSignal(ctx context.Context, cfg apiConfig, handler http.Handler, stdout io.Writer, stopRelay func()) error {
	srv := &http.Server{
		Addr:              cfg.addr,
		Handler:           handler,
