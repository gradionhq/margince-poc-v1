...
// posture); a bound path in, the cold-start/scrape/brief/growth-fit/reply set out.
func TestColdStartOptionsRespectsResolvedPath(t *testing.T) {
	...
	if got := coldStartOptions(modelPath, ""); len(got) != 6 {
		t.Fatalf(
			"coldStartOptions(bound path) = %d options, want 6 (cold-start, scrape, morning brief, account brief, growth fit, reply draft)",
			len(got))
	}
}
