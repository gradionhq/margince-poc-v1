...
// The mint flow's own backend: the POST answers with a token exactly once, and
// the caller decides whether it succeeds, so a refusal can be exercised without
// a second fixture.
function mintBackend(opts: { refuse?: boolean } = {}) {
  return vi.fn(async (input: RequestInfo | URL) => {
    const url = String(input instanceof Request ? input.url : input);
    const method = input instanceof Request ? input.method : "GET";
    if (url.endsWith("/v1/me")) {
...
// Minting is a credential-issuing flow whose result is shown exactly once, and
// it had no test at all while it was a row of controls inside the card. These
// hold what the drawer owes a reader: a named group of choices, a refusal it
// cannot submit past, the token surfaced where it cannot be missed, and a
// dialog that does not take that token away by closing itself.
describe("PassportCard — minting", () => {
  async function openDrawer() {
    render(<SettingsScreen tab="agents" />);
    const open = await screen.findByRole("button", { name: "Mint passport" });
  ...
  it("puts the form in a drawer, with the scopes as a named group", async () => {
    ...
    // The name field is a real label, not a span pointed at by aria-labelledby:
    // clicking the words has to focus the control.
    const name = within(dialog).getByLabelText("Agent name");
    await userEvent.click(within(dialog).getByText("Agent name"));
  ...
  it("shows the token once, moves focus to it, and does not close itself", async () => {
    vi.stubGlobal("fetch", mintBackend());
    const dialog = await openDrawer();
    await userEvent.type(within(dialog).getByLabelText("Agent name"), "Scout");
