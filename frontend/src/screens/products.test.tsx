...
function render(ui: ReactNode) {
  ...
  return rtlRender(<QueryClientProvider client={client}><LocaleProvider initial="en">{ui}</LocaleProvider></QueryClientProvider>);
}
const product = {
  id: "p-1", workspace_id: "w", name: "Consulting Day", sku: "CONS-DAY",
  unit: "day", unit_price_minor: 150000, currency: "EUR", default_tax_rate: 19,
  active: true, source: "manual", captured_by: "human:u1", version: 1,
...
describe("ProductsScreen", () => {
  it("renders products with money formatted from minor units", async () => {
    vi.stubGlobal("fetch", vi.fn(async () =>
      jsonResponse({ data: [product], page: { next_cursor: null, has_more: false } })));
  ...
  it("surfaces a 409 SKU-duplicate detail verbatim on create", async () => {
    const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (method === "POST") {
        return jsonResponse({ title: "conflict", detail: "sku already in use", code: "duplicate_sku" }, 409);
      }
