...
//
// It lives in the design system rather than beside its first caller because
// there is ONE mark: the shell's workspace chip and the conversation's own
// speaker are the same product saying its own name, and a second copy of this
// path data is a second mark the moment either is touched. Every fill reads
// `currentColor`, so the caller's colour is the mark's colour and the three
// stepped opacities survive it.
export function Logomark({ size = 19 }: Readonly<{ size?: number }>) {
  return (
    <svg
      viewBox="0 0 299 230"
      width={size}
