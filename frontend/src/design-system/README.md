...
| `MarginceCoreScene` | The product's one piece of AI identity: a glass ball of liquid with four dots suspended in it, in the closed eight-state vocabulary of the agent's work lifecycle (dormant · ingesting · reasoning · drafting · applied · flagged · disconnected · error). State is MOTION first — one formation each — and colour second. `aria-hidden`, and it carries no click. Callers pass `state` and never restyle; a different material is the `--coreC1/2/3` triple, never a second component. `margince-core-motion.ts` (pure placements) / `margince-core-engine.ts` (the loop) / `margince-core-feed.tsx` are its internals, not a caller's API | `margince-core.tsx` | ✅ |
...
| `Breadcrumb` | The trail that says where the reader is and how they got here: a `nav` landmark the caller names, an `<ol>`, one `<li>` per stop. It refuses the three things a hand-rolled trail gets wrong. The LAST stop is the current page and is never a link, even when the caller passes it an `href` — a link to the page you are already on is a control that does nothing. Separators are `aria-hidden` spans INSIDE the item they follow, never list items of their own (a three-stop trail would then announce as five things) and never CSS `content` (some screen readers speak it and some do not, so the same page reads differently to two people). And only the last stop gives way under pressure: ancestors are short labels the product chose, and shortening them would hide the way back, while the last is user data of unbounded length — `useTruncationTooltip` carries the rest of it on hover AND on focus. Takes no `className` or `style`: a trail restyled per screen is how the page head and the record page came to draw two different crumbs | `breadcrumb.tsx` | ✅ |
