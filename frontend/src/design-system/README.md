...
| **`ChoiceList`** | **One question, every answer readable AT REST — the radio GROUP the `Radio` atom never had. A `fieldset` with `role="radiogroup"`, a legend that can be hidden without going unnamed, and an optional `description` per answer that is part of the label (so the help line is clickable and is read with the option it belongs to).** Reach for it over `Select` for a decision a reader must weigh — a dropdown over two options makes somebody open a menu to discover the alternative — and over `SegmentedControl` the moment an option is a sentence rather than a tab name | `choicelist.tsx` | ✅ |
