...
| **`Switch`** | **A setting that writes when you flip it. A `Checkbox` states an intent something later submits; this IS the action, which is why it announces `role="switch"` and takes a `pending` state — the same contract as `Button`'s, `aria-disabled` and a mark rather than a dimmed control. A filter over a list is neither — that is a pressed button** | `switch.tsx` | ✅ |
